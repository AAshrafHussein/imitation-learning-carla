...
from psudo_auto_control import PsudoAutoController
...
def run_carla_client(args):
    ...
    report = {
        'num_episodes': args.num_episodes, #num of episodes for done on this run
        'controller_name': args.controller_name, # name of controller used in this run
        'distances': [],			# distanc traveld by car in this controller successfully
        'target_speed': args.target_speed,	# the taarget speed set to this controller
    }
	    ...
	    # In case of collision or stop episod fails and we don't sore data and restart it.
            if 'FAIL' in status:
                ...
                if args.store_data:
                	...
                	pd.DataFrame(log_dicts).to_csv('logs/{}_racetrack{}_log{}.csv'.format(args.controller_name, args.racetrack, episode), index=False)
                episode += 1
